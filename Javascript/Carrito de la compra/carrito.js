...
function iniciar(){
    ...
    if (almacenado != ""){ // Comprobamos si el carrito tenía algo almacenado 
...
function refrescar(){
    ...
    for (i=0; i < producto.length; i++ ) // Recorremos todos los elementos del carrito
    {
        ...
        if (producto['uds']>0){
            ...
            document.getElementById("carrito").innerHTML = texto // Añadimos el texto
            listaCarrito.appendChild(nodo);
            total = total + producto['uds']*producto['precio'];
        }
    }
    document.getElementById("datos").innerHTML = "Total: " + precio // Escribimos el total
}